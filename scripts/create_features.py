...
def main():
    ...
    features = pd.DataFrame({
        ...
        "num_songs": grouped.apply(lambda x: (x["page"] == "NextSong").sum()).values,
        "num_ads": grouped.apply(lambda x: (x["page"] == "Roll Advert").sum()).values,
        "thumbs_up": grouped.apply(lambda x: (x["page"] == "Thumbs Up").sum()).values,
        "thumbs_down": grouped.apply(lambda x: (x["page"] == "Thumbs Down").sum()).values,
        "help_events": grouped.apply(lambda x: (x["page"] == "Help").sum()).values,
        "error_events": grouped.apply(lambda x: (x["page"] == "Error").sum()).values,
        "downgrade_events": grouped.apply(lambda x: (x["page"] == "Downgrade").sum()).values,
    ...
    features["songs_per_session"] = features["num_songs"] / features["num_sessions"].replace(0, 1)
    features["ads_per_session"] = features["num_ads"] / features["num_sessions"].replace(0, 1)
